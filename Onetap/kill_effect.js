...
function render_effect()
{
    ...
    const inc_size = ((1 / get("Effect duration")) * Global.Frametime()) * 150

    alpha = clamp(alpha - inc_alpha, 0, 255);
    size = clamp(size - inc_size, 0, 150);

    const x = Global.GetScreenSize()[0], y = Global.GetScreenSize()[1];

    Render.GradientRect(0, 0, x, size, 0, [255, 128, 128, alpha], [255, 128, 128, 0]);
    Render.GradientRect(0, y - size, x, size, 0, [255, 128, 128, 0], [255, 128, 128, alpha]);
    Render.GradientRect(x - size, 0, size, y, 1, [255, 128, 128, 0], [255, 128, 128, alpha]);
    Render.GradientRect(0, 0, size, y, 1, [255, 128, 128, alpha], [255, 128, 128, 0]);
}
